...
export function CartProvider({ children }: { children: React.ReactNode }) {
    ...
    const removeFromCart = (cartItemId: number) => performCartAction(() => removeFromCartService(cartItemId));
    const updateQuantity = (cartItemId: number, newQuantity: number) => performCartAction(() => updateQuantityService(cartItemId, newQuantity));
    
    const totalItems = items.reduce((sum, item) => sum + item.quantidade, 0);

    const totalPrice = items.reduce((sum, item) => {
        const price = item.produto.is_oferta && item.produto.preco_oferta ? item.produto.preco_oferta : item.produto.preco;
